...
const storageKey = 'scy-toolbox-web'
const defaults = { nickname: 'SCY 用户', favorites: [] as string[], history: [] as HistoryEntry[], usageCount: 0 }
...
export const useToolboxStore = defineStore('toolbox', {
  ...
  actions: {
    ...
    persist() { if (import.meta.client && this.hydrated) window.localStorage.setItem(storageKey, JSON.stringify({ nickname: this.nickname, favorites: this.favorites, history: this.history, usageCount: this.usageCount })) },
    ...
    setNickname(nickname: string) { this.nickname = nickname.trim() || 'SCY 用户'; this.persist() },
  },
})
