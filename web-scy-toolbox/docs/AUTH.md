...
Web 账号仅使用单独创建的 CloudBase 环境和其中的 `web_users` 集合，不会调用小程序的云函数、环境或 `users` 集合。

## CloudBase

1. 创建新的 CloudBase 环境，并在 API Key 管理中创建服务端 API Key。
...
- `CLOUDBASE_ENV_ID`：新建的 Web CloudBase 环境 ID。
